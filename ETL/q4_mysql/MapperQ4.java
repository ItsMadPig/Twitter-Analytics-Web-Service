		...
		try {
			...
			while ((json = br.readLine()) != null) {
				if (!json.trim().equals("")) {
					...
					if (tweetTime == null || userId == null)
						continue;
					String tweetDate = tweetTime.substring(0, 10);

					String tweetId = inputJsonOject.get("id_str").getAsString();
					JsonArray hashtags = inputJsonOject.get("entities")
							.getAsJsonObject().get("hashtags").getAsJsonArray();
					if (hashtags != null) {
						for (int i = 0; i < hashtags.size(); i++) {
							String text = hashtags.get(i).getAsJsonObject()
									.get("text").getAsString();
							// English case-sensitive(0-9,A-Z,a-z) hashtags only
							if (isValidString(text))
								System.out.println(text + "_" + tweetDate
										+ "\t" + tweetId + "," + userId + ","
										+ tweetTime);
						}
					}
