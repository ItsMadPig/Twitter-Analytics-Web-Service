...
public class Q4 {
	...
	private static String queryMysql(String hashtag, Date startDate, Date endDate) {
        ...
        try {
        	...
        	PreparedStatement pstmt = con.prepareStatement("SELECT response FROM " + tableName + " force index (hashtag_date) WHERE  hashtag=? and tweetdate>=? and tweetdate<=? order by tweetdate");
